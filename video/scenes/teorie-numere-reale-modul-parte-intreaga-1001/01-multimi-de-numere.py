...
class MultimiDeNumere(VoiceoverScene):
    def construct(self):
        self.set_speech_service(EdgeTTSService(voice="ro-RO-AlinaNeural"), create_subcaption=True)
        self.opening()
        self.naturale()
        self.intregi()
        self.rationale()
    ...
    def intregi(self):
        ...
        with self.voiceover(
            ...
            self.play(Transform(self.head, head2), Transform(self.formula, formula2), run_time=1.2)
            self.wait_until_bookmark("axa")
            # A Transform between two NumberLines drops their number labels, so the axis is
            # swapped outright instead of morphed.
            self.play(FadeOut(self.line), FadeOut(self.dots), run_time=0.4)
    ...
    def rationale(self):
        ...
        with self.voiceover(
            text=(
                "Mergem mai departe la numerele raționale. "
                "<bookmark mark='def'/> Un număr rațional este o fracție, cu numărătorul întreg "
            ...
            self.play(Transform(self.head, head3), run_time=0.8)
            self.wait_until_bookmark("def")
            self.play(Transform(self.formula, formula3), run_time=1.4)
    ...
    def irationale(self):
        ...
        with self.voiceover(
            text=(
                "Nu toate numerele sunt raționale. "
                "<bookmark mark='def'/> Numerele iraționale sunt cele reale care nu sunt raționale. "
                "<bookmark mark='ex'/> Radical din doi, radical din trei și numărul pi sunt iraționale. "
                "<bookmark mark='note'/> Scrierea lor zecimală este infinită și nu se repetă niciodată."
            )
        ) as t:
            self.play(Transform(self.head, head4), run_time=0.8)
            self.wait_until_bookmark("def")
            self.play(Transform(self.formula, formula4), run_time=1.2)
            self.wait_until_bookmark("ex")
    ...
    def notatii(self):
        ...
        with self.voiceover(
            text=(
                ...
                "<bookmark mark='b'/> R plus înseamnă numerele reale pozitive, cu zero cu tot. "
                "<bookmark mark='c'/> R minus înseamnă numerele reale negative, tot cu zero."
            )
        ) as t:
